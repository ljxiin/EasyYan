  ...
  <section class="statistics-page">
    ...
    <template v-if="activeTab === 'overview'">
      ...
      <div class="overview-grid">
        ...
        <DataPanel title="知识点正确率 TOP10">
          <template #actions>
            <button type="button" class="detail-link" @click="knowledgeDialogVisible = true">查看详情 ›</button>
          </template>
    ...
    <template v-else-if="activeTab === 'mistakes'">
      ...
      <div class="mistake-bottom">
        ...
        <DataPanel title="需要重点关注的错题" link="查看全部错题"><StandardTable :columns="focusColumns" :rows="focusMistakes" /><Pagination :total="`共 ${profile.mistakes-profile.eliminated} 条`" :pages="String(Math.ceil((profile.mistakes-profile.eliminated) / 10))" /></DataPanel>
      </div>
    </template>
    ...
    <template v-else>
      ...
      <DataPanel class="records-panel">
        ...
        <Pagination :total="`共 ${profile.sessions} 条记录`" :pages="String(Math.ceil(profile.sessions / 10))" />
      </DataPanel>
    </template>
    ...
    >
      ...
      <div class="knowledge-detail-table">
        <div class="knowledge-detail-head">
          <span>排名</span><span>考点</span><span>所属科目</span><span>刷题数</span><span>正确率</span>
        </div>
    ...
    >
      ...
      <div class="question-detail-table">
        ...
        <div v-for="row in knowledgeQuestionRows" :key="row.id" class="question-detail-row">
          ...
          <span><button type="button" class="question-action">查看解析</button></span>
        </div>
      </div>
      <template #footer>
        <button type="button" class="dialog-close-btn" @click="questionDialogVisible = false">关闭</button>
        <button type="button" class="dialog-practice-btn">练习该知识点</button>
      </template>
    </el-dialog>
  </section>
</template>
...
const questionTemplates: Record<SubjectKey, string[]> = {
  calculus: ['求下列函数在指定点处的极限','判断函数在给定区间上的连续性','计算函数的一阶导数','求曲线在指定点处的切线方程','计算不定积分并验证结果','利用定积分求平面图形面积','求多元函数的偏导数','讨论函数极值并求极值点'],
  'linear-algebra': ['计算给定矩阵的行列式','求矩阵的逆矩阵','判断向量组的线性相关性','求矩阵的秩','解非齐次线性方程组','求矩阵的特征值','求特征向量并判断可对角化性','将二次型化为标准形'],
  probability: ['计算随机事件的概率','利用条件概率公式求解','求随机变量的分布函数','计算数学期望和方差','求二维随机变量的边缘分布','判断随机变量的独立性','利用中心极限定理估计概率','计算总体参数的置信区间']
}
const knowledgeQuestionRows = computed(() => {
  ...
  return questionTemplates[selectedSubject.value].map((question, index) => {
    ...
    return {
      ...
      question: `${selectedKnowledgeName.value}：${question}`,
...
function openKnowledgeQuestionDetail(row: Record<string, unknown>) {
  selectedKnowledgeName.value = String(row.knowledgeName || '').trim()
...
const mistakeKnowledge = computed(() => knowledgeRows.value.map((r,i)=>({rank:i+1,name:r.name,count:Math.max(3,Math.round((100-r.rate)*.75)),rate:Number((100-r.rate).toFixed(1)),delta:(i%3===0?'↑ ':'↓ ')+[6.3,5.1,2.4,3.2,1.6,2.1,1.8,.9,1.2,1][i]+'%'})))
...
const focusMistakes = computed(() => mistakeQuestions[selectedSubject.value].map((q,i)=>({question:q,point:knowledgeRows.value[i].name,times:i<3?'3次':'2次',date:['2024-05-29','2024-05-28','2024-05-25','2024-05-27','2024-05-24','2024-05-23'][i],action:'查看解析'})))
...
const recordTable = computed(() => calculusRecords.map((row,index)=>({
  date:row[0], mode:row[1], content:recordContent[selectedSubject.value][index],
...
.knowledge-detail-head,.knowledge-detail-row { display:grid; grid-template-columns:58px 1.6fr 1fr 82px 150px; align-items:center; min-height:42px; padding:0 14px; border-bottom:1px solid #edf0f7; font-size:12px; }
