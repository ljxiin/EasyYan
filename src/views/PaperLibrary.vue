<template>
  <section class="paper-page">
    ...
    <div class="filters">
      <el-select v-model="filters.subject" placeholder="全部科目"><el-option label="全部科目" value="" /><el-option label="高等数学" value="高等数学" /><el-option label="线性代数" value="线性代数" /><el-option label="概率论与数理统计" value="概率论与数理统计" /></el-select>
    ...
    <section class="panel paper-table">
      <div class="paper-row paper-head">
        <span><el-checkbox :model-value="allVisibleSelected" :indeterminate="partiallySelected" @change="toggleAllVisible" /></span><span>试卷名称</span><span>科目 ↕</span><span>来源 ↕</span><span>年份 ↕</span><span>题量 ↕</span><span>难度 ↕</span><span>正确率 ↕</span><span>练习进度 ↕</span><span>操作</span>
      </div>
      <div v-for="paper in filteredPapers" :key="paper.id" class="paper-row">
        ...
        <span><i class="subject-tag" :class="paper.subjectTone">{{ paper.subject }}</i></span>
      ...
      <footer>
        <span>共 {{ totalPaperCount }} 份试卷</span>
        <el-select model-value="10 条/页"><el-option label="10 条/页" value="10 条/页" /></el-select>
        <el-pagination v-model:current-page="page" layout="prev, pager, next" :total="totalPaperCount" :page-size="10" />
      </footer>
    </section>
  </section>
</template>
...
const filters = reactive({ subject: '', source: '', year: '', difficulty: '', type: '', favorite: '' })
const papers = ref([
...
function reset() { Object.assign(filters, { subject: '', source: '', year: '', difficulty: '', type: '', favorite: '' }); keyword.value = '' }
