<template>
  <section class="question-bank">
    <div class="subject-toolbar panel">
      <label>
        <span>考试科目：</span>
        <el-select v-model="selectedExamSubject" class="subject-select">
          <el-option v-for="item in examSubjectOptions" :key="item" :label="item" :value="item" />
        </el-select>
      </label>
      <label>
        <span>刷题科目：</span>
        <el-select v-model="selectedPracticeSubject" class="subject-select" @change="clearSelection">
          <el-option v-for="item in practiceSubjectOptions" :key="item" :label="item" :value="item" />
        </el-select>
      </label>
    </div>
    ...
    <el-dialog v-model="editDialogVisible" title="编辑题目" width="760px">
      <el-form label-position="top">
        ...
        <el-form-item v-if="isChoiceEdit" label="选项（支持 LaTeX）">
...
const selectedExamSubject = ref('数学（一）')
const selectedPracticeSubject = ref('高等数学')
...
const statSymbols = ['Σ', '◆', '✓', '★', '!']
const examSubjectOptions = ['数学（一）']
const practiceSubjectOptions = ['高等数学', '线性代数', '概率统计']
const typeOptions = ['单选题', '多选题', '填空题', '计算题', '应用题', '证明题']
const difficultyOptions = ['易', '中', '难']
const optionLabels = ['A', 'B', 'C', 'D', 'E', 'F']
const isChoiceEdit = computed(() => ['单选题', '多选题'].includes(editForm.type))
...
const filteredQuestionRows = computed(() => {
  ...
  return localQuestionRows.value.filter((row) => {
    const matchSubject = getPracticeSubject(row.no) === selectedPracticeSubject.value
    const matchChapter = selectedPracticeSubject.value === '高等数学' ? ids.includes(row.chapterId) : true
    ...
    return matchSubject && matchChapter && matchType && matchDifficulty && matchTag && matchKeyword
  })
})
...
function clearSelection() {
  selectedQuestionNos.value = []
}

function getPracticeSubject(no: number) {
  if ([6, 8].includes(no)) return '线性代数'
  if ([9].includes(no)) return '概率统计'
  return '高等数学'
}
...
function editQuestion(no: number) {
  const row = localQuestionRows.value.find((item) => item.no === no)
  if (!row) return
  const question = row as typeof row & { answer?: string; latexAnalysis?: string }
  Object.assign(editForm, {
    no: row.no,
    type: row.type,
    latexStem: row.latexStem ?? row.stem,
    options: row.options?.map((option) => option.replace(/^[A-F][.、]\s*/i, '')) ?? [],
    answer: question.answer ?? defaultAnswer(row.type),
    latexAnalysis: question.latexAnalysis ?? defaultAnalysis(row.no, row.type)
  })
...
function saveEditedQuestion() {
  const row = localQuestionRows.value.find((item) => item.no === editForm.no)
  if (!row) return
  const question = row as typeof row & { answer?: string; latexAnalysis?: string }

  const nextStem = editForm.latexStem.trim()

  if (!nextStem || !editForm.answer.trim() || !editForm.latexAnalysis.trim()) {
    ElMessage.warning('题目、正确答案和解析不能为空')
    return
  }
  if (isChoiceEdit.value && editForm.options.some((option) => !option.trim())) {
    ElMessage.warning('选择题的选项不能为空')
    return
  }

  row.latexStem = nextStem
  row.stem = nextStem.replace(/\$/g, '')
  if (isChoiceEdit.value) {
    row.options = editForm.options.map((option, index) => `${optionLabels[index]}. ${option.trim()}`)
  } else {
    delete row.options
  }
  question.answer = editForm.answer.trim()
  question.latexAnalysis = editForm.latexAnalysis.trim()

  editDialogVisible.value = false
  ElMessage.success(`第 ${row.no} 题已更新`)
}

function defaultAnswer(type: string) {
  if (type === '单选题' || type === '多选题') return 'A'
  if (type === '证明题') return '证明见解析'
  return '$0$'
}

function defaultAnalysis(no: number, type: string) {
  if (no === 1) return '$f^{\\prime}(x)=\\frac{-2x}{(1+x^2)^2}$，则 $f^{\\prime}(0)=0$。'
  if (type === '证明题') return '根据相关定义与定理逐步证明即可。'
  return '根据题意列式并化简，得到正确答案。'
}
...
.subject-select :deep(.el-select__wrapper) {
  ...
  padding: 0 18px;
  border-radius: 7px;
  box-shadow: 0 0 0 1px #dfe6f4 inset;
}

.subject-select :deep(.el-select__selected-item) {
  color: var(--ink);
  font-size: 18px;
  font-weight: 850;
}
...
.point-tag-list :deep(.el-tag) {
  max-width: 140px;
}

.point-tag-list :deep(.el-tag__content) {
  overflow: hidden;
